...
input double RMC_MinConfirmMagnitude = 0.02; // RMC must be at least this far from zero (of its -1..+1 range) to count as a real red/blue reading, not noise sitting on the zero line -- locked in from a dev-slice sweep of 0.00-0.20: win rate degrades monotonically as this rises (no sweet spot), so 0.02 is the smallest floor that still rejects a bare-noise reading like -0.0414
...
int OnInit()
{
   Print("=== ZEUS BUILD 2026-08-02-PORT | RC + Renko Momentum Cycle (RMC) dual-agreement direction | Synth removed ===");
   lastBarTime          = 0;
   ReadyToTrade         = false;
...
// GetLotSize
// +------------------------------------------------------------------+
double GetLotSize(int weightPct)
{
   ...
   double slTicks     = (StopLossPoints * Point) / tickSize;
...
void OpenTrade(int type, double entry, double candleClose,
               int tpPoints, string label, int weightPct)
{
   double vsl = (type == OP_BUY)
                ? NormalizeDouble(candleClose - StopLossPoints * Point, Digits)
                : NormalizeDouble(candleClose + StopLossPoints * Point, Digits);
   ...
   double lots   = GetLotSize(weightPct);
...
void OpenAll(int type)
{
   ...
   if(Trade1Enabled) OpenTrade(type, entry, candleClose, TP1, "T1", T1_Weight);
   if(Trade2Enabled) OpenTrade(type, entry, candleClose, TP2, "T2", T2_Weight);
   if(Trade3Enabled) OpenTrade(type, entry, candleClose, TP3, "T3", T3_Weight);
   if(Trade4Enabled) OpenTrade(type, entry, candleClose, TP4, "T4", T4_Weight);
...
void OnTick()
{
      ...
      CountTrades() == 0 && AllowNewEntries)
   {
      ...
      if(dirOK && brickOK)
      {
         if(entryDir > 0) OpenAll(OP_BUY);
         else              OpenAll(OP_SELL);
      }
   }
